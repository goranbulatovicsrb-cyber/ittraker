...
void ClientsPage::buildUi()
{
    ...
    auto* btnAdd = new QPushButton("➕  Nova mušterija");
    btnAdd->setObjectName("btnPrimary");
    m_btnEdit = new QPushButton("✏️  Izmeni");
...
void ClientsPage::populateTable(const QList<Client>& clients)
{
    ...
    m_table->setSortingEnabled(true);
    m_lblCount->setText(QString("%1 mušterija").arg(clients.size()));
    onSelectionChanged();
}
...
{
    if (text.isEmpty()) { populateTable(m_allClients); return; }
...
void ClientsPage::onDelete()
{
    ...
    auto ans = QMessageBox::question(this, "Potvrda brisanja",
        QString("Obrisati mušteriju \"%1\"?\nSvi poslovi vezani za ovu mušteriju bit će obrisani.").arg(c.name),
