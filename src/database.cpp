...
QList<WorkEntry> Database::overdueEntries(int days)
{
    QList<WorkEntry> list;
    QString cutoff = QDate::currentDate().addDays(-days).toString("yyyy-MM-dd");
    QString sql = R"(
        SELECT w.id, w.user_id, w.client_id, c.name, w.work_type, w.description,
               w.date, w.hours, w.price_per_hour, w.total_price, w.is_paid
        FROM work_entries w
        JOIN clients c ON c.id = w.client_id
        WHERE w.user_id=? AND w.is_paid=0 AND w.date <= ?
        ORDER BY w.date ASC
    )";
    QSqlQuery q(m_db);
    q.prepare(sql);
    ...
    while (q.next()) {
        ...
        w.date=QDate::fromString(q.value(6).toString(),"yyyy-MM-dd");
        w.hours=q.value(7).toDouble(); w.pricePerHour=q.value(8).toDouble();
        w.totalPrice=q.value(9).toDouble(); w.isPaid=false;
        list.append(w);
    }
    return list;
}
